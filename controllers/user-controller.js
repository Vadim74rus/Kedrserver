...
const UserController = {
  register: async (req, res) => {
    ...
    try {
      ...
      const avatarPath = await generateRandomAvatar(name);
      ...
      const user = await prisma.user.create({
        data: {
          ...
          balance: 100, // Добавляем поле balance с начальным значением 100
        },
      });
  ...
  updateUser: async (req, res) => {
    const { id } = req.params;
    const { email, name, dateOfBirth, bio, location, balance } = req.body;
    ...
    try {
      ...
      const user = await prisma.user.update({
        ...
        data: {
          ...
          balance: balance || undefined, // Добавляем поле balance
        },
      });
      res.json(user);
    } catch (error) {
...
async function generateRandomAvatar(name) {
  try {
    // Генерируем аватар с помощью Jdenticon
    ...
    // Сохраняем аватар на диск
    fs.writeFileSync(avatarPath, png);

    return avatarPath;
  } catch (error) {
    console.error("Error in generateRandomAvatar:", error);
    throw error;
  }
}
